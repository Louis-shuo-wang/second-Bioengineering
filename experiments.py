...
RES = {}
FIGDIR = 'C:/Users/lenovo/Downloads'
np.save(f'{FIGDIR}/xic.npy', np.array([12.473812]))
...
def exp1():
    ...
    for tag,s0,col in [('decay (s0>delta)',0.50,'C0'),('invade (s0<delta)',0.10,'C3')]:
        ...
        axes[0].semilogy(t_eval,uinf,col,label=tag)
        axes[1].plot(t_eval,umass,col,label=tag)
        # growth-rate panel: compare slope to prediction
        axes[2].plot(rate_pred,rate_meas,'o',color=col,ms=9,label=tag)
...
def exp3():
    ...
    for xi,col in [(0.0,'C0'),(0.6*xic,'C1'),(1.6*xic,'C3')]:
        ...
        axes[0].plot(np.sqrt(mu_cont)*L/np.pi,gc,col,lw=1.6,
                     label=fr'$\xi={xi:.1f}$')
    ...
    # simulation above onset, modal growth via DCT (Neumann cosine basis)
    ...
    for j in range(sol.y.shape[1]):
        u,v,w=split(sol.y[:,j],N)
        ck=np.abs(dct(v-v.mean(), type=2, norm='ortho'))  # cosine-mode amplitudes
        amps.append(ck)
    amps=np.array(amps)
    # measured modal growth rate in linear window (amplitudes above noise floor)
    meas=[]
    ...
    band=pred>-0.05   # near-marginal / unstable band where linear theory governs
    axes[1].axhspan(0,axes[1].get_ylim()[1],alpha=0.0)
    ...
    axes[1].set(xlabel='mode k',ylabel='growth rate',
                title=fr'(b) modal growth rates, $\xi={xi:.1f}$')
    axes[1].legend(fontsize=7.5)
    fig.tight_layout(); fig.savefig(f'{FIGDIR}/fig_exp3.pdf'); plt.close(fig)
    kstar=int(meas[np.argmax(pred),0])
    band_corr=float(np.corrcoef(pred[band],meas[band,1])[0,1])
    RES['exp3']=dict(xi_c=xic,k_star=kstar,band_corr=band_corr,
                     pred_kstar=float(pred.max()),
                     meas_kstar=float(meas[np.argmax(pred),1]))
    print('EXP3 xi_c=%.3f k*=%d band_corr=%.4f'%(xic,kstar,RES['exp3']['band_corr']))
